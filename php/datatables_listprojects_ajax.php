<?php
	 ...
	 */
	$aColumns = array('id','name','proj_status','locationname','locareaname','summary');
	...
	if ( $sWhere != "" )
	{
		  ...
		  projects.id AS projectsid,locationname,
                  FROM jacks,locations,locareas
		  LEFT OUTER JOIN locations ON projects.locationid=locations.id
		  LEFT OUTER JOIN locareas ON projects.locareaid=locareas.id
		  $sWhere";
	      $sth=db_execute($dbh,$sQueryCnt);
	      $rResultTotal=$sth->fetch(PDO::FETCH_ASSOC);
	...
	$sQuery = "
		  ...
		  projects.id AS projectid,
                  name,notes,locationid,locareaid,summary,
				  locations.name AS locationname,
                  locareas.areaname AS locareaname,
                  FROM
                  projects
		  LEFT OUTER JOIN locations ON projects.locationid=locations.id
		  LEFT OUTER JOIN locareas ON projects.locareaid=locareas.id
