...
class PDF_Label extends TCPDF{
	...
	function Add_Label($text_head, $text, $padding=3, $bordercolor=230, $img="",$imwidth=0,$imheight=0,
		...
		if (strstr($text,"ID:")) {
		  ...
		  $t=$text;
		  $t=preg_replace('/.*ID:.([0-9]+).*/','ID: \1 ',$t);
		  $t=preg_replace('/Model:.([^\n]*\n+)+.*/','',$t);
		  //$text=preg_replace('/ID:[0-9]+\n/','',$text);
		  $text=preg_replace('/ID:.[0-9]+\n/','',$text);
		  $text=preg_replace('/Name:.[A-Za-z0-9_-]+\n/','',$text);
		  $this->y+=1; //move name and ID 1 pixel down
		...
		if (strlen($barcode)) {
		  ...
		  $this->write2DBarcode($barcode, 'QRCODE,M', $X-1, $Y-1, $barcodesize,$barcodesize, $bstyle, 'N');
