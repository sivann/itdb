<?php
	...
	if ( $sWhere != "" )
	{
	      ...
	      $sQueryCnt = "
		  SELECT count(jacks.id) as count ,
		  jacks.id AS jacksid,locationname,
                  FROM jacks
		  JOIN users ON jacks.userid=users.id
		  LEFT OUTER JOIN locations ON jacks.locationid=locations.id
		  LEFT OUTER JOIN locareas ON jacks.locareaid=locareas.id
		  $sWhere";
	      $sth=db_execute($dbh,$sQueryCnt);
	      $rResultTotal=$sth->fetch(PDO::FETCH_ASSOC);
...
FROM
  jacks
  LEFT OUTER JOIN departments ON (jacks.departmentsid = departments.id)
  LEFT OUTER JOIN locations ON (jacks.locationid = locations.id)
  LEFT OUTER JOIN vlans ON (jacks.vlanid = vlans.id)
  LEFT OUTER JOIN locareas ON (jacks.locareaid = locareas.id)
  		  $sWhere
		  $sOrder
		  $sLimit
