-- Restore missing data after migration - CORRECTED VERSION
-- This script restores data that failed during the initial migration
-- Uses actual column names from the backup database
...
-- Restore software data with correct column mapping
-- backup.software has: id, invoiceid, slicenseinfo, stype, manufacturerid, stitle, sversion, sinfo, purchdate, licqty, lictype
-- current.software has: id, stitle, sversion, slicense, scomments, url, slicensetype, scat, manufacturerid
INSERT INTO software (id, stitle, sversion, slicense, scomments, url, slicensetype, scat, manufacturerid)
SELECT
    s.id,
    s.stitle,
    s.sversion,
    s.slicenseinfo as slicense,  -- map slicenseinfo to slicense
    s.sinfo as scomments,        -- map sinfo to scomments
    '' as url,                   -- no url in backup
    s.stype as slicensetype,     -- map stype to slicensetype
    '' as scat,                  -- no scat in backup
...
-- Restore invoices with correct column mapping
-- backup.invoices has: id, number, date, vendorid, buyerid, description
-- current.invoices has: id, date, vendorid, buyerid, comments, totalcost
...
SELECT
    ...
    i.description as comments,  -- map description to comments
    0.0 as totalcost           -- no totalcost in backup
...
-- Restore actions with correct column mapping
-- backup.actions has: id, itemid, actiondate, description, invoiceinfo, isauto, entrydate
-- current.actions has: id, itemid, actiondate, description, userid
...
SELECT
    ...
    a.actiondate,
    a.description,
    NULL as userid  -- no userid in backup actions
...
SELECT 'Data restoration complete with foreign key constraints enabled!' as status;