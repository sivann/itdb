...
for ($i=0;$i<count($itypes);$i++) {
  $typeid2name[$itypes[$i]['id']]=$itypes[$i]['description'];
}
...
<div id="tab1" class="tab_content">
  ...
  <td class='tdtop'>
    ...
    for ($i=0;$i<count($itypes);$i++) {
      $dbid=$itypes[$i]['id']; $itype=$itypes[$i]['description']; $s="";
    ...
    ?>
      ...
      <select name='rack_units'>
      <option value=''><?php te("Select");?></option>

    <?php 
    for ($i=1;$i<45;$i++) {
      $s="";
      if ($rack_units=="$i") $s=" SELECTED ";
    ...
    ?>
      ...
      <tr> <td class='tdt'><?php te("S/N");?>:</td><td><input type=text value='<?php echo $serial_number?>' name='serial_number'></td> </tr>
      <tr> <td class='tdt'><?php te("S/N 2");?>:</td><td><input type=text value='<?php echo $serial_number_2?>' name='serial_number_2'></td> </tr>
      <tr>
	...
	$dbid=$statustypes[$i]['id']; $itype=$statustypes[$i]['name']; $s="";
...
//linked items
$sql="SELECT items.id,manufacturer_id,model,item_type_id,serial_number || ' '||serial_number_2 ||' ' || serial_number_3 as sn,label,dns_name,users.username AS username ".
$sth=db_execute($dbh,$sql);
...
//not linked items
$sql="SELECT items.id,manufacturer_id,model,item_type_id, serial_number || ' '||serial_number_2 ||' ' || serial_number_3 as sn,label,function,dns_name, users.username AS username".
...
<div id='tab5' class='tab_content'>
  ...
  $sql=" SELECT COALESCE((SELECT software_id from item2soft WHERE item_id='$id' AND software_id=software.id ),0) islinked , ".
       ...
       " ORDER BY islinked desc,manufacturer_id,title,version ";
  $sth=db_execute($dbh,$sql);
  ?>
...
<div id='tab6' class='tab_content'>
  ...
  $sql=" SELECT COALESCE((SELECT contract_id FROM contracts_items WHERE item_id='$id' AND contract_id=contracts.id ),0) islinked , ".
       ...
       " ORDER BY islinked desc,contractor_id,title";
  $sth=db_execute($dbh,$sql);
  ?>
  <div style='margin-left:auto;margin-right:auto;' class='scrltblcontainer2'>
