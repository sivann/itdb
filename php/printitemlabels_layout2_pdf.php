...
require("../init.php");
require_once('PDF_Label_layout2.php');
...
for ($row=1;$row<=$rows;$row++) {
  ...
  for ($col=1;$col<=$cols;$col++) {
    ...
    // Trim after 20 character to fit Avery Zweckform ZWE3651
    $desc="$agtitle $model";
    $desc=mb_substr($desc,0,20);
    $desc=trim($desc);
    //$sn=strlen($r['sn'])>0?$r['sn']:$r['sn3'];
	
	$sn=strlen($r['sn'])>0?$r['sn']:$r['sn'];
    $sn2=strlen($r['sn2'])>0?$r['sn2']:$r['sn2'];
    $sn3=strlen($r['sn3'])>0?$r['sn3']:$r['sn3'];

// Original Label Layout Start
/*    
    $labeltext="";
...
*/
// Original Label Layout End

// New Layout:
	$labeltext="";
    $labeltext.=sprintf("ID:$id\n");
    if (strlen($desc)) { $labeltext.="Model: ".$desc."\n"; }
    else $labeltext.=sprintf("Model: \n");
    if (strlen($sn)) { $labeltext.=sprintf("SN: $sn\n\n"); }
    else $labeltext.=sprintf("SN: \n\n");
    if (strlen($sn2)) { $labeltext.=sprintf("Inventory:\n$sn2\n"); }
    else $labeltext.=sprintf("Inventory: \n\n");
    if (strlen($sn3)) { $labeltext.=sprintf("$sn3\n"); }
    else $labeltext.=sprintf("\n");
    $labeltext=rtrim($labeltext);
